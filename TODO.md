...
- **追加バリデーション** ― 優先度: 高 / 難度: 小
  以下のフィールドを `validate()` に追加する。テストも合わせて書く。

  | フィールド | チェック内容 | 優先度 |
  | --- | --- | --- |
  | `id` | 使用可能文字（英数字・ハイフン・アンダースコアのみ）。スペースや `/` はディレクトリ名として壊れる | 高 |
  | `authors` | 空リストまたは省略を禁止。BibTeX 出力や表示に直接影響する | 高 |
  | `scope` | `domestic` / `international` のみ許容。それ以外だとフィルタが機能しない | 高 |
  | `registered_at` | `date` と同じ形式（`YYYY-MM-DD`）。同じロジックで検証可能 | 中 |
  | `files` の各要素 | `path` か `url` の少なくとも一方が必要 | 中 |
  | `paper_type` | `full` / `short` のみ許容（journal のみ対象） | 低 |
  | `status`（patent） | `applied` / `granted` のみ許容 | 低 |
